...
begin
    ...
    FOR i IN 1..6
    LOOP
        account := LPAD(4680000000+i*100,10,'0');
        premium := 15;
        INSERT INTO PISD.T_PISD_INSURANCE_CONTRACT
              ...
              VALUES(entity                      , branch_id                   , account                      , '7'                          , '7'                           , 8                   , '01'                  , 1                   , '0007168999', 'P029999'           , 'P029999'            , branch_id                 , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),0), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),0), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12), 12                           , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12), '90008603' , '00110486840190021967', 'N'                   , 1                    , 1                   , 50.00         , premium*12                   , 'PEN'      , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12) , 20000.00      , '08'            , 0             , 'PC'                       , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),1), 'N'                        , 'FOR'                         , 'FOR'             , 'P029801'       , TO_DATE(sysdate,'DD/MM/YYYY'), 'P029801'    , TO_DATE(sysdate,'DD/MM/YYYY'), 'S'                        , 0                , 999.00                     , 0                            , 600.00                       , '8308'                      , 'N'                           , '923123'                );
        FOR j IN 1..1
        LOOP
          ...
          VALUES( entity                      , branch_id                   , account                      , LPAD(j,2,'0')    , 2.80                       , 5.36                      , '0000075020'               , 'PEN'      , premium                       , TO_DATE(sysdate,'DD/MM/YYYY'), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1)+30 , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), TO_DATE(sysdate,'DD/MM/YYYY'), '00'                          , '31010005418'               , 'C'                , '00110486840190021967', 'PC'              , 0                     , 'INC'                        , case when j =1 then 'COB' WHEN j=1 then  'IMP' else 'INC' end, 'P029801'       , TO_DATE(sysdate,'DD/MM/YYYY'), 'P029801'    , TO_DATE(sysdate,'DD/MM/YYYY'), branch_id           , 0                      , 15.00             , 0                            , 180.00                       , branch_id            , 1                   , branch_id    , '111716888920'+i   ) ;
        END LOOP;
