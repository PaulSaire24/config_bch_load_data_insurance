...
begin
    ...
    LOOP
        ...
        LOOP
          ...
          VALUES( entity                      , branch_id                   , account                      , LPAD(j,2,'0')    , 0                          , 0                           , '0000075050'+i               , 'PEN'      , premium                       , TO_DATE(sysdate,'DD/MM/YYYY'), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1)+30 , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), TO_DATE(sysdate,'DD/MM/YYYY'), '00'                          , '31010005710'+i               , 'C'                , '00110486840190021967', 'PC'              , 0                     , 'INC'                        , case when j =1 then 'COB' WHEN j=1 then  'IMP' else 'INC' end, 'P029804'       , TO_DATE(sysdate,'DD/MM/YYYY'), 'P029801'    , TO_DATE(sysdate,'DD/MM/YYYY'), branch_id           , 0                      , 179.01             , 0                            , 5000                       , branch_id            , 1                   , branch_id     , '10099338140'+i   ) ;
         END LOOP;
