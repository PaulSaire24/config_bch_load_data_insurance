...
begin
    ...
    LOOP
        ...
        INSERT INTO PISD.T_PISD_INSURANCE_CONTRACT
            ...
            VALUES(entity                      ,branch_id                   ,account                       ,'7'                          ,'7'                           ,1                   , '02'                  ,1                   ,'970111' ,41.93                         ,0                 ,'P029798'           ,'P029798'            ,'0814'                    , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),0) , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),0)     ,  ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12)     ,12                            , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12)    ,'87780638'  ,'00110241700199004116',1                    ,1                   ,41.93        ,'USD'        , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),0)  , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12)           ,9000.00      , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),1)   , TO_DATE(sysdate,'DD/MM/YYYY') ,'P029801'      ,TO_DATE(sysdate,'DD/MM/YYYY'), 12                          , 'FOR'                         , 'FOR'              , '2101'                  , 'N');
        FOR j IN 1..12
        LOOP
            ...
            VALUES( entity                      , branch_id                   , account                      , LPAD(j,2,'0')    , 0                         , 0                           , '0000075090'+i               , 'PEN'      , premium                       , TO_DATE(sysdate,'DD/MM/YYYY'), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1)+30 , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),j-1), TO_DATE(sysdate,'DD/MM/YYYY'), case when j =1 then '00' else '02' end , '31010007110'+i               , 'C'                , '00110486840190021967', 'PC'              , 0                     , 'INC'                        , case when j =1 then 'COB' WHEN j=1 then  'IMP' else 'INC' end, 'P029808'       , TO_DATE(sysdate,'DD/MM/YYYY'), 'P029805'    , TO_DATE(sysdate,'DD/MM/YYYY'), branch_id           , 0                      , 32.61             , 0                            , case when j =1 then 32.61 else 0 end    , branch_id            , 1                   , branch_id    , '10099328'||LPAD(j,3,'0')   ) ;
        END LOOP;
