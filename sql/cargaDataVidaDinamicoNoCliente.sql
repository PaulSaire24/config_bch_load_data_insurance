...
begin
    ...
    LOOP
        ...
        INSERT INTO PISD.T_PISD_INSURANCE_CONTRACT
                    (INSURANCE_CONTRACT_ENTITY_ID, INSURANCE_CONTRACT_BRANCH_ID, INSRC_CONTRACT_INT_ACCOUNT_ID, CONTRACT_FIRST_VERFN_DIGIT_ID, CONTRACT_SECOND_VERFN_DIGIT_ID,INSURANCE_PRODUCT_ID,POLICY_QUOTA_INTERNAL_ID, INSURANCE_MODALITY_TYPE, INSURANCE_COMPANY_ID, POLICY_ID       , INSURANCE_MANAGER_ID, INSURANCE_PROMOTER_ID, CONTRACT_MANAGER_BRANCH_ID, CONTRACT_INCEPTION_DATE                    , INSURANCE_CONTRACT_START_DATE              , INSURANCE_CONTRACT_END_DATE                 , INSRNC_VALIDITY_MONTHS_NUMBER, INSURANCE_POLICY_END_DATE                   , CUSTOMER_ID, DOMICILE_CONTRACT_ID  , CARD_ISSUING_MARK_TYPE, ISSUED_RECEIPT_NUMBER, PAYMENT_FREQUENCY_ID, PREMIUM_AMOUNT, SETTLE_PENDING_PREMIUM_AMOUNT, CURRENCY_ID, LAST_INSTALLMENT_DATE                       , INSTALLMENT_PERIOD_FINAL_DATE                , INSURED_AMOUNT, BENEFICIARY_TYPE, RENEWAL_NUMBER, CTRCT_DISPUTE_STATUS_TYPE, PERIOD_NEXT_PAYMENT_DATE                   , ENDORSEMENT_POLICY_IND_TYPE, INSRNC_CO_CONTRACT_STATUS_TYPE, CONTRACT_STATUS_ID, CREATION_USER_ID, CREATION_DATE                , USER_AUDIT_ID, AUDIT_DATE                   , INSUR_PENDING_DEBT_IND_TYPE, TOTAL_DEBT_AMOUNT, PREV_PEND_BILL_RCPTS_NUMBER, SETTLEMENT_VAR_PREMIUM_AMOUNT, SETTLEMENT_FIX_PREMIUM_AMOUNT, INSURANCE_COMPANY_PRODUCT_ID, AUTOMATIC_DEBIT_INDICATOR_TYPE, BIOMETRY_TRANSACTION_ID )
              VALUES(entity                      , branch_id                   , account                      , '7'                          , '7'                           , 9                   ,'0814000005471'+i        , '02'                  , 1                   , '1300029403' + i, 'P029999'           , 'P029999'            , branch_id                 , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),0), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),0), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12), 12                           , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12), '90008603' , '00110486840190021967', 'N'                   , 1                    , 1                   , 50.00         , premium*12                   , 'PEN'      , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12), ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),12) , 20000.00      , '08'            , 0             , 'PC'                       , ADD_MONTHS(to_Date(my_date,'DD/MM/YYYY'),1), 'N'                     , 'PEN'                         , 'FOR'             , 'P029801'       , TO_DATE(sysdate,'DD/MM/YYYY'), 'P029801'    , TO_DATE(sysdate,'DD/MM/YYYY'), 'S'                        , 0                , 999.00                     , 0                            , 600.00                       , '8046'                      , 'N'                           , '923123'                );
        FOR j IN 1..1
        LOOP
